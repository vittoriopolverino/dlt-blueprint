...
@dataclass
class Email(Pipeline):
    spark: SparkSession
    datalake_folder: str
    table_name: str = field(default='email')
    primary_key: list[str] = field(default_factory=lambda: ['activityid'])
    ...
    def create_bronze_table(cls, datalake_folder: str, table_name: str, primary_key: list[str]) -> None:
        is_pk_not_null = super().create_pk_is_not_null_condition(primary_key=primary_key)
        ...
        def bronze_table():
            df = dlt.read(f'scd1_{table_name}').withColumn(
                'plaintext_description', regexp_replace('description', '<[^>]*>|&nbsp;', ' ')
            )
            return super(Email, cls).check_duplicates_from_df(dataframe=df, primary_key=primary_key)
